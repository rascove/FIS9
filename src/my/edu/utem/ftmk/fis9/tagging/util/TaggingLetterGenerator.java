 ...
 */
public class TaggingLetterGenerator
{
	...
	public static void generate(File file, Tagging tagging, District district, Range range, int level) throws Exception
	{
		...
		if (tagging.isOpen())
		{
			document.addTitle("MAKLUMAT KERJA-KERJA PENANDAAN POKOK");
			...
			calendar.setTimeInMillis(tagging.getTaggingID());
			...
			detail.addCell(new Phrase(":", normal));
			detail.addCell(new Phrase(tagging.getForestName(), normal));
			detail.addCell(new Phrase("", normal));
			detail.addCell(new Phrase("No. Kompatmen/Blok", normal));
			detail.addCell(new Phrase(":", normal));
			detail.addCell(new Phrase(tagging.getComptBlockNo(), normal));
			detail.addCell(new Phrase("", normal));
			detail.addCell(new Phrase("Keluasan", normal));
			detail.addCell(new Phrase(":", normal));
			detail.addCell(new Phrase(tagging.getArea() + " ha", normal));
			detail.addCell(new Phrase("", normal));
			detail.addCell(new Phrase("Tahun", normal));
			detail.addCell(new Phrase(":", normal));
			detail.addCell(new Phrase("" + tagging.getYear(), normal));
			...
			if (level > 1)
			{
				...
				detail.addCell(new Phrase(":", normal));
				detail.addCell(new Phrase(sdf.format(tagging.getStartDate()) + " - " + sdf.format(tagging.getEndDate()), normal));
			}
			...
			if (tagging.getTenderNo() != null)
			{
				...
				detail.addCell(new Phrase(":", normal));
				detail.addCell(new Phrase(tagging.getTenderNo(), normal));
			}
			...
			if (level == 1)
			{
				...
				document.add(new Paragraph(15f, "MAKLUMAT KERJA-KERJA PENANDAAN POKOK", bold));
			}
			else if (level == 2)
			{
				document.add(new Paragraph(15f, "\nPenolong Pegawai\nPejabat Hutan Daerah Renj " + range.getName() + ",\n" + range.getAddress() + "\n" + range.getStateName() + "\n\nTuan,\n\n", normal));
				document.add(new Paragraph(15f, "ARAHAN KERJA-KERJA PENANDAAN POKOK", bold));
			}
			else if (level == 3)
			{
				document.add(new Paragraph(15f, "\n" + tagging.getTeamLeaderName() + "\nPejabat Hutan Daerah Renj " + range.getName() + ",\n" + range.getAddress() + "\n" + range.getStateName() + "\n\nTuan,\n\n", normal));
				document.add(new Paragraph(15f, "LANTIKAN SEBAGAI KETUA PASUKAN UNTUK KERJA-KERJA PENANDAAN POKOK", bold));
			}

			document.add(new Paragraph(15f, "\nDengan hormatnya saya di arah merujuk kepada perkara di atas.\n\n2. Dimaklumkan maklumat-maklumat penandaan pokok adalah seperti berikut:\n ", normal));
			document.add(detail);
			...
			if (level == 1)
				document.add(new Paragraph(18f, "Nama: " + tagging.getCreatorName(), normal));
			else if (level == 2)
		...
		{
			document.addTitle("PENENTUAN HAD TEBANGAN");
		}
