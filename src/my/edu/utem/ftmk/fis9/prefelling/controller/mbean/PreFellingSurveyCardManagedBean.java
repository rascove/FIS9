...
public class PreFellingSurveyCardManagedBean extends AbstractManagedBean<PreFellingSurveyCard>
{
	...
	public PreFellingSurveyCardManagedBean()
	{
		try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
		{
			...
			int stateID = user.getStateID(), designationID = user.getDesignationID(), endYear = pFacade.getPreFellingSurveyYearRange()[1];
			...
			if (stateID == 0)
			{
				if (designationID == 0)
				{
					preFellingSurveys = pFacade.getPreFellingSurveys(0, endYear);
					uptype = "ccoo";
				}
				else
				{
					preFellingSurveys = pFacade.getPreFellingSurveys(user, 0, endYear);
					accessLevel = 6;
					downtype = "ccoc";
				}
			}
			else
			{
				...
				{
					preFellingSurveys = pFacade.getPreFellingSurveys(state, 0, endYear);
				...
				else
				{
					...
					if (district != null)
					{
						preFellingSurveys = pFacade.getPreFellingSurveys(district, 0, endYear);
					...
					else
					{
						...
						if (range != null)
						{
							preFellingSurveys = pFacade.getPreFellingSurveys(range, 0, endYear);
							accessLevel = 4;
							uptype = "ccof";
							downtype = "ccor";
						}
	...
	public void setPreFellingSurveyCard(PreFellingSurveyCard preFellingSurveyCard)
	{
	...
	public void setPreFellingSurveyRecord(PreFellingSurveyRecord preFellingSurveyRecord)
	{
	...
	public void setPreFellingTraditionalSurveyCards(PreFellingSurveyCard[] traditionalPreFellingSurveyCards)
	{
	...
	public void setPreFellingCuttingOptions(ArrayList<PreFellingCuttingOption> preFellingCuttingOptions)
	{
	...
	public void setAddPreFellingSurveyRecordOperation(boolean addPreFellingSurveyRecordOperation)
	{
	...
	public void handlePreFellingSurveyIDChange()
	{
		...
		try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
		{
			...
			if (preFellingSurvey.getStartDate() != null && preFellingSurvey.getEndDate() != null)
				workingDates = sdf.format(preFellingSurvey.getStartDate()) + " sehingga " + sdf.format(preFellingSurvey.getEndDate());
			else
			...
			{
				District district = mFacade.getDistrict(preFellingSurvey.getDistrictID());
				downloaded = user.getStaffID().equals(district.getOfficerID());
			}

			if (accessLevel == 7)
				recommendations = pFacade.getCuttingOptionRecommendations(preFellingSurvey);
		}
		catch (SQLException e)
		{
			e.printStackTrace();
	...
	public void handleOpen()
	{
		if (addOperation)
		{
			...
			model.setPreFellingSurveyID(preFellingSurvey.getPreFellingSurveyID());
	...
	public void handleOpenPreFellingSurveyRecord()
	{
		if (addPreFellingSurveyRecordOperation)
		{
			...
			preFellingSurveyRecord.setPreFellingSurveyRecordID(System.currentTimeMillis());
			preFellingSurveyRecord.setPreFellingSurveyCardID(model.getPreFellingSurveyCardID());
		}
		else
		{
			preFellingSurveyRecord = (PreFellingSurveyRecord) copy(model.getPreFellingSurveyRecords(), preFellingSurveyRecord);
	...
	public void handleOpenTraditional()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			preFellingSurveyCard.setPreFellingSurveyRecords(preFellingSurveyRecords);

			for (int j = 0; j < 10; j++)
			{
				PreFellingSurveyRecord preFellingSurveyRecord = new PreFellingSurveyRecord();

				preFellingSurveyRecord.setPreFellingSurveyRecordID(current + i * 10 + j);
	...
	public void preFellingSurveyCardEntry()
	{
		try (PreFellingFacade facade = new PreFellingFacade())
		{
			...
			/*for (Geology geology : geologies)
				if (geology.getGeologyID() == preFellingSurveyCard.getGeologyID())
			...
			finalizeModelEntry(addOperation ? facade.addPreFellingSurveyCard(model, true) : facade.updatePreFellingSurveyCard(model), addOperation, facade, "kad bancian, ID " + model.getPreFellingSurveyCardID(), ", kerana no. garisan dan petak telah direkodkan sebelumnya", models, model);
			model = null;
		}
		catch (SQLException e)
		{
	...
	public void preFellingSurveyRecordEntry()
	{
		try (PreFellingFacade facade = new PreFellingFacade())
		{
			...
			{
				if (species.getSpeciesID() == preFellingSurveyRecord.getSpeciesID())
				{
					preFellingSurveyRecord.setSpeciesCode(species.getCode());
					preFellingSurveyRecord.setSpeciesName(species.getName());
					preFellingSurveyRecord.setSpeciesTypeID(species.getSpeciesTypeID());
				}
			}

			for (LogQuality logQuality : logQualities)
			{
				if (logQuality.getLogQualityID() == preFellingSurveyRecord.getLogQualityID())
				{
					preFellingSurveyRecord.setLogQuality(logQuality.getCode());
					preFellingSurveyRecord.setLogQualityName(logQuality.getName());
				}
			}

			for (Fertility fertility : fertilities)
			{
				if (fertility.getFertilityID() == preFellingSurveyRecord.getFertilityID())
				{
					preFellingSurveyRecord.setFertility(fertility.getCode());
					preFellingSurveyRecord.setFertilityName(fertility.getName());
				}
			}

			for (VineSpreadth vineSpreadth : vineSpreadths)
			{
				if (vineSpreadth.getVineSpreadthID() == preFellingSurveyRecord.getVineSpreadthID())
				{
					preFellingSurveyRecord.setVineSpreadth(vineSpreadth.getCode());
					preFellingSurveyRecord.setVineSpreadthName(vineSpreadth.getName());
			...
			if (addPreFellingSurveyRecordOperation)
			{
				finalizeModelEntry(facade.addPreFellingSurveyRecord(preFellingSurveyRecord, true), addPreFellingSurveyRecordOperation, facade, "rekod bancian, ID " + preFellingSurveyRecord.getPreFellingSurveyRecordID(), ", kerana rekod bancian telah direkodkan sebelumnya", model.getPreFellingSurveyRecords(), preFellingSurveyRecord);
			...
			{
				finalizeModelEntry(facade.updatePreFellingSurveyRecord(preFellingSurveyRecord), addPreFellingSurveyRecordOperation, facade, "rekod bancian, ID " + preFellingSurveyRecord.getPreFellingSurveyRecordID(), ", kerana rekod bancian telah direkodkan sebelumnya", model.getPreFellingSurveyRecords(), preFellingSurveyRecord);
	...
	public void preFellingSurveyRecordDelete()
	{
		try (PreFellingFacade facade = new PreFellingFacade())
		{
			if (facade.deletePreFellingSurveyRecord(preFellingSurveyRecord) != 0)
			{
				addMessage(FacesMessage.SEVERITY_INFO, null, preFellingSurveyRecord + " berjaya dipadamkan.");
				model.getPreFellingSurveyRecords().remove(preFellingSurveyRecord);
				log(facade, "Padam rekod bancian, ID " + preFellingSurveyRecord.getPreFellingSurveyRecordID());
			}
			else
				addMessage(FacesMessage.SEVERITY_WARN, null, preFellingSurveyRecord + " tidak dapat dipadamkan.");
	...
	public void preFellingSurveyRecordEntryTraditional()
	{
		try (PreFellingFacade facade = new PreFellingFacade())
		{
			...
			{
				ArrayList<PreFellingSurveyRecord> preFellingSurveyRecords = traditionalSurveyCard.getPreFellingSurveyRecords();
				...
				for (PreFellingSurveyRecord preFellingSurveyRecord : preFellingSurveyRecords)
				{
					if (preFellingSurveyRecord.getSpeciesID() != 0)
					{
						...
						if (facade.addPreFellingSurveyRecord(preFellingSurveyRecord, true) != 0)
						{
							count++;

							model.getPreFellingSurveyRecords().add(preFellingSurveyRecord);
							log(facade, "Tambah rekod bancian, ID " + preFellingSurveyRecord.getPreFellingSurveyRecordID());
						}
					}
				}
			}
	...
	public void validate()
	{
		try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
		{
			...
			for (PreFellingSurveyCard preFellingSurveyCard : models)
			{
				if (preFellingSurveyCard.getInspectorID() == null && preFellingSurveyCard.getInspectionDate() != null)
				{
					count++;
					...
					if (pFacade.updatePreFellingSurveyCard(preFellingSurveyCard) != 0)
						total++;
				}
			}
			...
			addMessage(FacesMessage.SEVERITY_INFO, null, (total == 0 ? "Tiada" : total) + " kad bancian berjaya disahkan.");
			...
			if (total != 0)
			{
				...
				new EmailSender().send(true, "Kad Bancian Inventori Hutan Sebelum Tebangan - " + preFellingSurvey.getForestName() + " " + preFellingSurvey.getComptBlockNo(), "Kad bancian Inventori Hutan Sebelum Tebangan (Pre-Felling) telah disahkan oleh " + user.getName() + " untuk:<br/><br/><table border='0'><tr><td>- Hutan simpan</td><td>:</td><td>" + preFellingSurvey.getForestName() + "</td></tr><tr><td>- No. kompartmen/sub kompartmen</td><td>:</td><td>" + preFellingSurvey.getComptBlockNo() + "</td></tr></table><br/>Sila log masuk ke FIS9 untuk tindakan anda seterusnya.", recipient);
			}
		}
		catch (SQLException e)
		{
	...
	public void prepareDownload()
	{
		...
		for (PreFellingSurveyCard preFellingSurveyCard : models)
		{
			if (preFellingSurveyCard.getInspectorID() == null)
			{
				...
				ArrayList<PreFellingSurveyCard> lineSurveyCard = map.get(lineNo);
	...
	public void prepareReport()
	{
		try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
		{
			...
			{
				PreFellingReport preFellingReport = new PreFellingReport(preFellingSurvey, mFacade.getTimberGroups(), mFacade.getCuttingOptions(state), logQualities);
	...
	{
		percentage = "Jumlah kad dipilih: " + (selectedLines == null || selectedLines.length == 0 ? 100 : Math.round(selectedLines.length * 100d / map.size())) + "%";
	}

	public void handleDrop(DragDropEvent event)
	{
		PreFellingCuttingOption preFellingCuttingOption = (PreFellingCuttingOption) event.getData();

		if (!preFellingCuttingOptions.contains(preFellingCuttingOption))
			preFellingCuttingOptions.add(preFellingCuttingOption);
		else
			addMessage(FacesMessage.SEVERITY_INFO, null, "Opsyen had tebangan sudah disyorkan sebelumnya.");
	}
	...
	public void recommend()
	{
		if (preFellingCuttingOptions != null)
		{
			try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
			{
				...
				{
					PreFellingCuttingOption preFellingCuttingOption = preFellingCuttingOptions.get(i);

					preFellingCuttingOption.setRank(i + 1);
					pFacade.addPreFellingCuttingOption(preFellingCuttingOption);
					recommendations.add(preFellingCuttingOption.getCuttingOptionID());
				}

				preFellingSurvey.setRecommended(true);
				addMessage(FacesMessage.SEVERITY_INFO, null, "Opsyen had tebangan berjaya disyorkan.");

				Staff user = getCurrentUser();
				State state = mFacade.getState(user.getStateID());

				new EmailSender().send(true, "Penetapan Had Batas Tebangan - " + preFellingSurvey.getForestName() + " " + preFellingSurvey.getComptBlockNo(), "Had batas tebangan Inventori Hutan Sebelum Tebangan (Pre-Felling) telah disyorkan oleh " + user.getName() + " untuk:<br/><br/><table border='0'><tr><td>- Hutan simpan</td><td>:</td><td>" + preFellingSurvey.getForestName() + "</td></tr><tr><td>- No. kompartmen/sub kompartmen</td><td>:</td><td>" + preFellingSurvey.getComptBlockNo() + "</td></tr></table><br/>Sila log masuk ke FIS9 untuk tindakan anda seterusnya.", state.getDirectorID());
			}
			catch (SQLException e)
			{
				e.printStackTrace();
	...
	public StreamedContent download()
	{
		...
		String name = "LaporanBancian_" + preFellingSurvey.getForestName().replaceAll(" ", "") + "_" + preFellingSurvey.getComptBlockNo() + "_" + preFellingSurvey.getYear() + ".pdf";
		...
		try
		{
			PreFellingReportGenerator.generate(file, preFellingSurvey, vineSpreadths, resams, fertilities);
	...
	public StreamedContent downloadLetter()
	{
		...
		try (MaintenanceFacade facade = new MaintenanceFacade())
		{
			...
			if (preFellingSurvey.getPreFellingReport() == null)
				preFellingSurvey.setPreFellingReport(new PreFellingReport(preFellingSurvey, facade.getTimberGroups(), facade.getCuttingOptions(state), facade.getLogQualities()));

			PreClosingLetterGenerator.generate(file, preFellingSurvey, treeLimit, state, getCurrentUser(), recommendations);
	...
	public StreamedContent download(boolean pdf)
	{
		...
		String name = "KadBancian_" + preFellingSurvey.getForestName().replaceAll(" ", "") + "_" + preFellingSurvey.getComptBlockNo() + "_" + preFellingSurvey.getYear() + "_" + host + "." + (pdf ? "pdf" : downtype), type = null;
		File file = new File(external.getRealPath("/") + "files/pre-f/" + name);
		StreamedContent content = null;
		...
		try
		{
			if (pdf)
			{
				try (MaintenanceFacade mFacade = new MaintenanceFacade(); PreFellingFacade pFacade = new PreFellingFacade();)
				{
					...
					Tender tender = mFacade.getTender(preFellingSurvey.getTenderNo());
					Contractor contractor = null;

					for (RegenerationSpecies regenerationSpecies : regenerationSpeciesList)
					{
						String species = regenerationSpecies.getName().toLowerCase(), code = regenerationSpecies.getCode(), symbol = "C";

						if (code != null)
						{
							if (code.equals("0106") || code.equals("0103") || code.equals("0105") || code.equals("0102"))
								symbol = "S";
					...
					map.put("0201", "M");
					PreFellingSurveyCardGenerator.generate(file, preFellingSurvey, contractor, map);
					...
					if (lineList != null)
					{
						...
						for (PreFellingSurveyCard preFellingSurveyCard : temp)
						{
							...
							pFacade.updatePreFellingSurveyCard(preFellingSurveyCard);
						}
					}
				}
	...
	public synchronized void upload(FileUploadEvent event)
	{
		...
		if (file != null)
		{
			try (ObjectInputStream ois = new ObjectInputStream(new GZIPInputStream(file.getInputstream())); PreFellingFacade facade = new PreFellingFacade();)
			{
				if (ois.readInt() != 22)
				...
				if (preFellingSurvey.equals(this.preFellingSurvey))
				{
					...
					ArrayList<PreFellingSurveyCard> currentSurveyCards = this.preFellingSurvey.getPreFellingSurveyCards();
					...
					if (filename.endsWith("ccof") || filename.endsWith("ccoc"))
					{
						if (adfor == null && dfo == null)
						{
							...
							save(facade, currentSurveyCards, preFellingSurveyCards, false);
							preFellingSurvey.setPreFellingSurveyCards(currentSurveyCards);
							preFellingSurveys.set(preFellingSurveys.indexOf(this.preFellingSurvey), preFellingSurvey);
							...
							addMessage(FacesMessage.SEVERITY_WARN, null,
									preFellingSurvey + " tidak dapat ditambahkan kerana sesi bancian yang dimuat naik tidak sah.");
					}
					...
					else if (filename.endsWith("ccoo"))
					{
						...
						{
							facade.updatePreFellingSurvey(preFellingSurvey);
							save(facade, currentSurveyCards, preFellingSurveyCards, true);
							preFellingSurvey.setPreFellingSurveyCards(currentSurveyCards);
							preFellingSurveys.set(preFellingSurveys.indexOf(this.preFellingSurvey), preFellingSurvey);
					...
					addMessage(FacesMessage.SEVERITY_WARN, null,
							preFellingSurvey + " tidak dapat ditambahkan kerana maklumat sesi bancian yang dimuat naik tidak sama dengan sesi bancian yang dipilih.");
			}
			catch (Exception e)
			{
				e.printStackTrace();
	...
	private void save(PreFellingFacade facade, ArrayList<PreFellingSurveyCard> currentPreFellingSurveyCards, ArrayList<PreFellingSurveyCard> preFellingSurveyCards, boolean strict) throws SQLException
	{
		...
		for (PreFellingSurveyCard preFellingSurveyCard : preFellingSurveyCards)
		{
			...
			if (facade.addPreFellingSurveyCard(preFellingSurveyCard, false) != 0)
			{
				totalSurveyCard++;
				int index = currentPreFellingSurveyCards.indexOf(preFellingSurveyCard);
				...
				if (index == -1)
				{
					...
					log(facade, "Tambah kad bancian, ID " + preFellingSurveyCard.getPreFellingSurveyCardID());
				}
				else
				{
					PreFellingSurveyCard temp = currentPreFellingSurveyCards.get(index);
					currentPreFellingSurveyRecords = temp.getPreFellingSurveyRecords();

					log(facade, "Kemaskini kad bancian, ID " + preFellingSurveyCard.getPreFellingSurveyCardID());
					preFellingSurveyCard.setPreFellingSurveyCardID(temp.getPreFellingSurveyCardID());
					currentPreFellingSurveyCards.set(index, preFellingSurveyCard);
				}

				for (PreFellingSurveyRecord preFellingSurveyRecord : preFellingSurveyRecords)
				{
					/*Integer nID = map.get(preFellingSurveyRecord.getSpeciesID());

					if (nID != null)
						preFellingSurveyRecord.setSpeciesID(nID);

					if (preFellingSurveyRecord.getPlotTypeID() > 6)
						preFellingSurveyRecord.setPlotTypeID(7);*/

					preFellingSurveyRecord.setPreFellingSurveyCardID(preFellingSurveyCard.getPreFellingSurveyCardID());

					try
					{
						if (facade.addPreFellingSurveyRecord(preFellingSurveyRecord, false) != 0)
						{
							totalRecord++;
							index = currentPreFellingSurveyRecords.indexOf(preFellingSurveyRecord);

							log(facade, "Tambah rekod bancian, ID " + preFellingSurveyRecord.getPreFellingSurveyRecordID());

							if (index == -1)
								currentPreFellingSurveyRecords.add(preFellingSurveyRecord);
							else
								currentPreFellingSurveyRecords.set(index, preFellingSurveyRecord);
						}
					}
					catch (Exception e)
					{
						System.err.println(preFellingSurveyRecord + "\t" + preFellingSurveyRecord.getSpeciesID());
					}
				}

				sort(currentPreFellingSurveyRecords);
				preFellingSurveyCard.setPreFellingSurveyRecords(currentPreFellingSurveyRecords);
			}
		}
		...
		if (totalSurveyCard != 0 || totalRecord != 0)
			addMessage(FacesMessage.SEVERITY_INFO, null,
					totalSurveyCard + " kad bancian dan " + totalRecord + " rekod bancian berjaya ditambahkan.");
		else
			addMessage(FacesMessage.SEVERITY_INFO, null,
