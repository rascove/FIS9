 ...
 */
public class LaporanKutipanHarianResitDariTarikhMulaHinggaTarikhAkhirGenerator
{
			...
			ArrayList<String[]> receipts, int reportType) throws Exception
	{
		...
		document.addKeywords("Laporan Kutipan Harian Mengikut Resit");
		document.addAuthor("JPNS");
		document.addCreator("JPNS");
		document.addTitle("LAPORAN KUTIPAN HARIAN MENGIKUT RESIT");
		...
		PdfPCell status = new PdfPCell(new Phrase("STATUS", titleFont));
		...
		status.setBorderWidthBottom(0.5f);
		...
		status.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
		...
		for (String current : criterias)
		{
			...
			PdfPTable content = new PdfPTable(new float[] {0.05f, 0.1f, 0.1f,
					0.1f, 0.1f, 0.1f, 0.2f, 0.1f, 0.15f});
			...
			aboutInner.addCell(new Phrase("TARIKH KUTIPAN", titleFont));
			aboutInner.addCell(new Phrase(":", titleFont));
			aboutInner.addCell(new Phrase(current, contentFont));
			aboutInner.completeRow();
	...
	public static void main(String[] args)
	{
		try (RevenueFacade rFacade = new RevenueFacade();)
		{
			...
			ArrayList<String[]> receipts = rFacade
					.getReceiptsString(start.getTime(), end.getTime(), 1);

			LaporanKutipanHarianResitDariTarikhMulaHinggaTarikhAkhirGenerator
					.generate(new File("incomebyreceipt.pdf"), start.getTime(),
							end.getTime(), receipts, 1);
		}
		catch (Exception e)
		{
			e.printStackTrace();
