 ...
 */
public class LaporanBagiPembatalanHasilDariTarikhMulaHinggaTarikhAkhirGenerator
{
	...
	public static void generate(File file, Date startDate, Date endDate,
			ArrayList<String[]> listOfCancelledTransactions) throws Exception
	{
		...
		document.addKeywords("Laporan Bagi Pembatalan Hasil");
		document.addAuthor("JPNS");
		document.addCreator("JPNS");
		document.addTitle("LAPORAN BAGI PEMBATALAN HASIL");

		DecimalFormat df = new DecimalFormat("#,###,##0.00");
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		PdfPTable content = new PdfPTable(
				new float[] {0.15f, 0.15f, 0.15f, 0.55f, 0.15f});
		PdfPCell[] headers = new PdfPCell[] {
				new PdfPCell(new Phrase("LAPORAN PEMBATALAN KUTIPAN DARI "
						+ sdf.format(startDate) + " HINGGA "
						+ sdf.format(endDate), titleFont)),
				new PdfPCell(new Phrase("NO. TRANSAKSI", titleFont)),
				new PdfPCell(new Phrase("TARIKH TRANSAKSI", titleFont)),
				new PdfPCell(new Phrase("TARIKH BATAL", titleFont)),
				new PdfPCell(new Phrase("ITEM / SEBAB", titleFont)),
		...
		for (String[] row : listOfCancelledTransactions)
		{
			if (!row[0].equals(current))
			{
				...
				PdfPCell header = new PdfPCell(
						new Phrase(map1.get(current) + " BATAL", titleFont));

				header.setColspan(5);
				content.addCell(header);
			}

			double amount = df.parse(row[5]).doubleValue();
			...
			content.addCell(new PdfPCell(new Phrase(row[4], contentFont)));
			content.addCell(value);
		}

		PdfPCell footer1 = new PdfPCell(new Phrase("JUMLAH BESAR " + map1.get(current), titleFont));
		PdfPCell footer2 = new PdfPCell(new Phrase(
	...
	public static void main(String[] args)
	{
		...
		{
			Date startDate = new GregorianCalendar(2018, 6, 1).getTime();
			Date endDate = new GregorianCalendar(2018, 6, 31).getTime();
			...
			LaporanBagiPembatalanHasilDariTarikhMulaHinggaTarikhAkhirGenerator
					.generate(new File("cancelledtransaction.pdf"), startDate,
							endDate, listOfCancelledTransactions);
		}
		catch (Exception e)
		{
			e.printStackTrace();
