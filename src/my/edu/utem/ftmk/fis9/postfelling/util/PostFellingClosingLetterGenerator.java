 ...
 */
public class PostFellingClosingLetterGenerator
{
	...
	public static void generate(File file, PreFellingSurvey preFellingSurvey, TreeLimit treeLimit, State state, Staff staff, ArrayList<Integer> recommendations) throws Exception
	{
		...
		limit.addCell(new Phrase("Anggaran peratus Kaum Damar dirian asal", normal));
		...
		limit.addCell(new Phrase("Anggaran peratus Kaum Damar dirian tinggal", normal));
		...
		{
				new PdfPCell(new Phrase("Damar", bold9)),
				new PdfPCell(new Phrase("Bukan Damar", bold9)),
				new PdfPCell(new Phrase("Damar", bold9)),
				new PdfPCell(new Phrase("Bukan Damar", bold9)),
		...
		document.add(table3);
		document.add(new Paragraph("\nNOTA: Peratus KAUM DAMAR yang berdiameter +30 cm dalam dirian asal = " + dfp.format(preFellingReport.getOriginalStandRatio()), size9));
