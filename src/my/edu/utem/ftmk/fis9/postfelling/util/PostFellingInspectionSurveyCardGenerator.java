 ...
 */
public class PostFellingInspectionSurveyCardGenerator
{
	...
	public static void generate(File file, PostFellingSurvey postFellingSurvey, Contractor contractor, LinkedHashMap<String, String> map, boolean all) throws Exception
	{
		...
		{
			if (!all)
				if(!postFellingSurveyCard.isInspection()) continue;
			
			if (postFellingSurveyCards.indexOf(postFellingSurveyCard) != 0)
				document.newPage();
			...
			{
				String code = postFellingSurveyRecord.getInspectionSpeciesCode(), symbol = map.get(code == null ? "" : code);
				int logQuality = postFellingSurveyRecord.getInspectionLogQualityID(), fertilityID = postFellingSurveyRecord.getInspectionFertilityID(), vineNo = postFellingSurveyRecord.getInspectionVine();
				int treeStatus = postFellingSurveyRecord.getInspectionTreeStatusID();
				double diameter = (postFellingSurveyRecord.getInspectionDiameter()==null ? 0: postFellingSurveyRecord.getInspectionDiameter());
				...
				if (postFellingSurveyRecord.getPlotTypeID() == 1 && diameter>0)
				{
				...
				else if (postFellingSurveyRecord.getPlotTypeID() == 2 && diameter>0)
				{
				...
				else if (postFellingSurveyRecord.getPlotTypeID() == 3 && diameter>0)
				{
				...
				else if (postFellingSurveyRecord.getPlotTypeID() == 5 && diameter>0)
				{
				...
				else if (postFellingSurveyRecord.getPlotTypeID() == 6 && (postFellingSurveyRecord.getInspectionSaplingQuantity()>0 || postFellingSurveyRecord.getInspectionSeedlingQuantity()>0) )
				{
					petak4.add(postFellingSurveyRecord);

					countA += postFellingSurveyRecord.getInspectionSaplingQuantity();
					countE += postFellingSurveyRecord.getInspectionSeedlingQuantity();
				}
			}

			if (postFellingSurveyCard.getInspectionBertam() >= 5)
				type2 += "BT ";
			
			if (postFellingSurveyCard.getInspectionBamboo() >= 5) 
				type2 += "B ";

			if (postFellingSurveyCard.getInspectionPalm() >= 9)
				type2 += "P ";
			
			if (postFellingSurveyCard.getInspectionResamID() >=2)
				type2 += "R ";

			if (postFellingSurveyCard.getInspectionGingerID() >= 2)
				type2 += "H ";
			
			if (postFellingSurveyCard.getInspectionBananaID() >= 2)
				type2 += "S ";
			...
			PdfPCell[] maklumat = new PdfPCell[] {
					...
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionForestType(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionSoilStatus(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionAspect(), size9)),
					new PdfPCell(new Phrase("" + postFellingSurveyCard.getInspectionSlope(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionSlopeLocation(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionElevation(), size9)),
					new PdfPCell(new Phrase("" + postFellingSurveyCard.getInspectionBertam(), size9)),
					new PdfPCell(new Phrase("" + postFellingSurveyCard.getInspectionBamboo(), size9)),
					new PdfPCell(new Phrase("" + postFellingSurveyCard.getInspectionPalm(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionResam(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionGinger(), size9)),
					new PdfPCell(new Phrase(postFellingSurveyCard.getInspectionBanana(), size9))
			};
			...
			for (int i = 0; i < max1; i++)
			{
				...
				if (i < sizeU1)
				{
					...
					contentB1s = new PdfPCell[] {
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSpeciesCode() == null ? "" : postFellingSurveyRecord.getInspectionSpeciesCode(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSpeciesName(), size7)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionDiameter(), size9)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionLogQuantity(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionLogQuality(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionTreeStatus(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSilara(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionDominance(), size9)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionVine(), size9))
			...
			String inspectorName = (postFellingSurveyCard.getInspectorName()==null ? "-" : postFellingSurveyCard.getInspectorName());
			...
			//headBDesc2s[1].setColspan(4);
			//headBDesc2s[3].setColspan(3);
			//headBDesc2s[4].setColspan(3);
			...
			for (int i = 0; i < max2; i++)
			{
				...
				if (i < sizeP3) 
				{
					...
					contentB4s = new PdfPCell[] {
							...
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSpeciesName(), size7)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionDiameter(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionFertility(), size9)),
				...
				if (i < sizeP4)
				{
					...
					contentB6s = new PdfPCell[] {
							...
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSpeciesCode() == null ? "" : postFellingSurveyRecord.getInspectionSpeciesCode(), size9)),
							new PdfPCell(new Phrase(postFellingSurveyRecord.getInspectionSpeciesName(), size7)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionSaplingQuantity(), size9)),
							new PdfPCell(new Phrase("" + postFellingSurveyRecord.getInspectionSeedlingQuantity(), size9))
					};
	...
	int getValidRecordSize(ArrayList<PostFellingSurveyRecord> postFellingSurveyRecords) {
		
		int size = 0;
		for (PostFellingSurveyRecord record: postFellingSurveyRecords)
		{
			if (record.getInspectionDiameter()>0 || record.getInspectionSaplingQuantity()>0 || record.getInspectionSeedlingQuantity()>0)
				size++;
		}
		return size;
	}
