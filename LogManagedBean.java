	...
	public LogManagedBean() {
		try (MaintenanceFacade mFacade = new MaintenanceFacade(); TaggingFacade tFacade = new TaggingFacade();) {
			...
			if (stateID == 0) 
			{
				...
				if (logSizes != null) logSizeExist = true;
			} 
			else 
			{
				...
				if (state.getDirectorID().equals(staffID) || designationID == 6 || designationID == 12) 
				{
				...
				else 
				{
					...
					if(hallOfficers.size() == 0)
					{
						...
						if (district != null && !staffID.equals(district.getClerk1ID())
								&& !staffID.equals(district.getClerk2ID()) && !staffID.equals(district.getClerk3ID())
								&& accessLevel == 2)
		...
		try (MaintenanceFacade mFacade = new MaintenanceFacade();
			...
			if (!logSizeExist) {
				addMessage(FacesMessage.SEVERITY_WARN, null,
						"Kemasukan data tual balak tidak boleh dilaksanakan kerana maklumat saiz balak bagi negeri sesi penandaan yang dipilih tidak didaftarkan.");
			} else {
